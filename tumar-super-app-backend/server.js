...
// Helper: ensure market_returns table exists
async function ensureMarketReturnsTable(connection) {
    await connection.execute(`
        CREATE TABLE IF NOT EXISTS market_returns (
            ...
            photos_json LONGTEXT NOT NULL DEFAULT '[]',
...
// POST /api/market/return/create — покупатель создаёт заявку на возврат
app.post('/api/market/return/create', authenticateToken, async (req, res) => {
    const userId = req.user.userId;
    const { order_ref, amount, reason, photos } = req.body;
    if (!order_ref || !reason || !photos || photos.length < 2) {
        return res.status(400).json({ success: false, message: 'Укажите заказ, причину и минимум 2 фото' });
    }
    if (photos.length > 10) {
        return res.status(400).json({ success: false, message: 'Максимум 10 фотографий' });
    }
    let connection;
    try {
        connection = await pool.getConnection();
        await ensureMarketReturnsTable(connection);
        await ensureMarketPurchasesTable(connection);
        const [existing] = await connection.execute(
            'SELECT id FROM market_returns WHERE user_id = ? AND order_ref = ?', [userId, order_ref]);
        if (existing.length) {
            return res.status(400).json({ success: false, message: 'Заявка на возврат уже существует для этого заказа' });
        }
        const parsedAmount = amount ? parseFloat(amount) : 0;
        await connection.execute(
            'INSERT INTO market_returns (user_id, order_ref, amount, reason, photos_json) VALUES (?, ?, ?, ?, ?)',
            [userId, order_ref, parsedAmount, reason.trim(), JSON.stringify(photos)]);
...
// GET /api/market/return/my — покупатель видит свои возвраты
app.get('/api/market/return/my', authenticateToken, async (req, res) => {
    const userId = req.user.userId;
    let connection;
    try {
        connection = await pool.getConnection();
        await ensureMarketReturnsTable(connection);
        const [rows] = await connection.execute(
            'SELECT id, order_ref, amount, reason, status, created_at FROM market_returns WHERE user_id = ? ORDER BY created_at DESC',
            [userId]);
        res.json({ success: true, returns: rows });
    } catch (err) {
        console.error('Return my error:', err);
        res.status(500).json({ success: false, message: 'Ошибка загрузки возвратов' });
    } finally {
        if (connection) connection.release();
    }
});

// GET /api/market/return/all — продавец видит все возвраты
app.get('/api/market/return/all', authenticateToken, async (req, res) => {
    let connection;
    try {
        connection = await pool.getConnection();
        await ensureMarketReturnsTable(connection);
        const [rows] = await connection.execute(
            `SELECT r.id, r.order_ref, r.amount, r.reason, r.photos_json, r.status, r.created_at,
                    u.phone_number
...
// POST /api/market/return/accept — продавец принимает заявку (→ COURIER_PENDING)
app.post('/api/market/return/accept', authenticateToken, async (req, res) => {
    const { return_id } = req.body;
    if (!return_id) return res.status(400).json({ success: false, message: 'return_id обязателен' });
    let connection;
    try {
        connection = await pool.getConnection();
        await ensureMarketReturnsTable(connection);
        const [rows] = await connection.execute(
            'SELECT id, status FROM market_returns WHERE id = ?', [return_id]);
        if (!rows.length) return res.status(404).json({ success: false, message: 'Заявка не найдена' });
        if (rows[0].status !== 'CREATED') {
            return res.status(400).json({ success: false, message: 'Нельзя принять заявку в текущем статусе' });
        }
        await connection.execute('UPDATE market_returns SET status = "COURIER_PENDING" WHERE id = ?', [return_id]);
        res.json({ success: true });
    } catch (err) {
...
// POST /api/market/return/refund — продавец возвращает деньги покупателю
app.post('/api/market/return/refund', authenticateToken, async (req, res) => {
    const { return_id } = req.body;
    if (!return_id) return res.status(400).json({ success: false, message: 'return_id обязателен' });
    let connection;
    try {
        ...
        await ensureMarketReturnsTable(connection);
        const [rows] = await connection.execute(
            'SELECT id, user_id, amount, status FROM market_returns WHERE id = ? FOR UPDATE', [return_id]);
        if (!rows.length) {
            await connection.rollback();
        ...
        if (rows[0].status === 'REFUNDED') {
            await connection.rollback();
            return res.status(400).json({ success: false, message: 'Деньги уже возвращены' });
        }
        const { user_id, amount } = rows[0];
        const [balRows] = await connection.execute('SELECT currency FROM balances WHERE user_id = ?', [user_id]);
        const currency = balRows.length ? (balRows[0].currency || 'KZT') : 'KZT';
        await connection.execute(
            'UPDATE balances SET balance = balance + ?, updated_at = NOW() WHERE user_id = ?', [amount, user_id]);
        await connection.execute(
            'INSERT INTO transactions (sender_id, recipient_id, amount, currency, transaction_type, description) VALUES (NULL, ?, ?, ?, ?, ?)',
            [user_id, amount, currency, 'MARKET_REFUND', 'Возврат товара — Tumar Market']);
        await connection.execute('UPDATE market_returns SET status = "REFUNDED" WHERE id = ?', [return_id]);
        await connection.commit();
        res.json({ success: true, refunded: parseFloat(amount) });
    } catch (err) {
        if (connection) await connection.rollback();
...
// POST /api/market/return/update-status — обновление статуса возврата
app.post('/api/market/return/update-status', authenticateToken, async (req, res) => {
    const userId = req.user.userId;
    const { return_id, status } = req.body;
    const allowed = ['IN_TRANSIT', 'PENDING_DECISION'];
    ...
    try {
        ...
        await ensureMarketReturnsTable(connection);
        const [rows] = await connection.execute(
            'SELECT id, user_id FROM market_returns WHERE id = ?', [return_id]);
        if (!rows.length) return res.status(404).json({ success: false, message: 'Заявка не найдена' });
        if (rows[0].user_id !== userId) {
            return res.status(403).json({ success: false, message: 'Нет доступа' });
        }
        await connection.execute('UPDATE market_returns SET status = ? WHERE id = ?', [status, return_id]);
        res.json({ success: true });
    } catch (err) {
        console.error('Return update-status error:', err);
